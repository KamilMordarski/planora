...
```powershell
.venv\Scripts\python.exe tools\prepare_update.py --repo TWOJ_LOGIN/NAZWA_REPO --note "Pierwsze wydanie Planory"
```

4. Zapisz i wyślij wygenerowany `update.json` do gałęzi `main`.
5. W ustawieniach Planory wklej:

```text
https://raw.githubusercontent.com/TWOJ_LOGIN/NAZWA_REPO/main/update.json
```

6. Włącz opcję sprawdzania aktualizacji przy uruchomieniu.
